...
\begin{description}
  ...
  \item[Get indexed elements as tuples (index, el) from iterable:]
  \ttt{enumerate(iterable)}
  \item[Same as above, with index starting at <startval>:]
  \ttt{enumerate(iterable, startval)}
  \item[Append <list2> to end of list:] \ttt{foo.extend(<list2>)}
\end{description}
...
\begin{description}
  ...
  \item[Iterate though keys:] \ttt{for key in foo.keys()}
  \item[Iterate though values:] \ttt{for value in foo.values()}
...
\begin{description}
  ...
  \item[\ttt{numvals} Evenly spaced numbers over range:] \ttt{np.linspace(beg, end,
    numvals)}
\end{description}
