...
\begin{itemize}
  ...
  false.
  \item Boldface capital letters are matrices, \eeg $\mA = \mU \mW \mV^\top$
...
\begin{itemize}
  ...
  \theta_j x_j \equiv \theta^\top x$, where $x_0 = 1$.
  \item Cost function
  \begin{equation}
  ...
  \begin{equation}
    \vec{\theta} = \left(\mX \mX^\top\right)^{-1} \mX\, \vec{y}
  \end{equation}
...
with deriv
\begin{equation}
  \frac{\partial}{\partial \theta_j} l(\theta) = x_j^\top (y - h_\theta (x))
\end{equation}
...
\begin{itemize}
  ...
  $\mbf{C_X} = \frac{1}{m}\mX\mX^\top$.
  \item \tbf{via SVD:} Define $\mY = \frac{1}{\sqrt{m}} \mX^\top$, and take its SVD
  \ref{subsec:SVD}, which is $\mY = \mU \mW \mV^\top$. Then $\mU^\top$ is $n \times m$
...
\begin{align}
  p(\vec{y})         &= \mathcal{N} (\vec{y} |\mA \vec{\mu} + \vec{b}, \mL^{-1}
                        + \mA \mLambda^{-1} \mA^\top ) \\
  p(\vec{x}|\vec{y}) &= \mathcal{N} ( \vec{x} | \mSigma \{ \mA^\top \mL
                        (\vec{y} - \vec{b}) + \mLambda \vec{\mu} \}, \mSigma)
\end{align}
where
\begin{equation}
  \mSigma \equiv \left(\mLambda + \mA^\top \mL \mA \right)^{-1}
\end{equation}


\subsection{Singular value decomposition}
\label{subsec:SVD}
An $M \times N$ matrix $\mA$ can be written as $\mU\mW\mV^\top$, where $\mU$ is
$M \times N$, $\mW$ is $N \times N$ and diagonal, and $\mV$ is $N \times N$ and
orthonormal.

If $\mA$ is square, $\mV$ are its eigenvectors, and $\mW$ are its eigenvalues...
...
\begin{equation}
  \frac{\partial}{\partial \vec{x}} \left( \vec{x}^{\top} \vec{a} \right) =
  \frac{\partial}{\partial \vec{x}} \left( \vec{a}^{\top} \vec{x} \right) =
  \vec{a} 
\end{equation}
...
Introduce prior $p(\theta ; \alpha) = \exp(-\alpha \theta^\top \theta)$. Find $\theta$
