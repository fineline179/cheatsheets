...
\begin{itemize}
  ...
  \item $\mathbb{E}_{\theta}$ : Expectation value (with repect to some parameter
...
\begin{align}
  \mathbb{E}_{\v{w}}[\v{y}] &= \mbbb{X}\, \mathbb{E}_{\v{w}}[\v{w}] = \v{0} \\
  \trm{cov}[\v{y}]  &= \mathbb{E}_{\v{w}}[\v{y} \v{y}^T]
                      = \mbbb{X}\, \mathbb{E}_{\v{w}}[\v{w} \v{w}^T]\, \mbbb{X}^T
...
\end{equation}
where $\v{t}$ is now $m+1$-dimensional, and the $(m+1) \times (m+1)$ matrix
...
From \cite{BISHOP}, section 2.3.3. NB This is known as a \tbfit{linear Gaussian model}.
...
$\mA = \mU \mD \mU^{-1} = \mU \mD \mU^T$, with $\mU$ an orthogonal\footnote{An
  ...
  columns are linearly indepdendent, but also normalized.} matrix with columns $\v{u}_i$
equal to $\mA$'s eigenvectors, and $\mD$ a diagonal matrix of $\mA$'s eigenvalues,
$\lambda_i$.
