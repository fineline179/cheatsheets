...
\begin{itemize}
  ...
  \ref{subsec:SVD}, which is $\mY = \mU \mW \mV^T$. Then $\mU^T$ is $n \times m$ matrix
  that projects $\mX$ onto its principal components, $\mW$ is the matrix of principal
  comp values, and $\mV$ is orthonormal matrix of the principal
...
\label{subsec:SVD}
An $M \times N$ matrix $\mA$ can be written as $\mU\mW\mV^T$, where $\mU$ is
