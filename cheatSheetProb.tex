...
\begin{equation}
  p(x;\eta) = b(x) \exp \lb \eta^T T(x) - a(\eta) \rb
\end{equation}
where
\begin{itemize}
  \item $\eta$ is the \tbfit{natural} or \tbfit{canonical parameter}
  \item $T(x)$ is the \tbfit{sufficient statistic}
  \item $a(\eta)$ is the \tbfit{log partition function}
  \item $b(x)$ determines the distribution at $\eta = 0$
\end{itemize}

\TODO{more detail about the above terms. \eeg the $\eta$ are the sources or external
  fields.}
